...
const Chat = {
  ...
  mutations: {
    updateCurrentMsg(state, payload) {
      const {
        chatType,
        chatId
      } = payload;
      state.currentMsg = `${chatType}_${chatId}`;
    },
    ...
    updateMsgList(state, payload) {
      ...
      if (chatType === "chatroom" && !bySelf) { // 聊天室消息去重处理
        ...
        // state.currentMsgs = Object.assign({}, state.msgList[chatType][params.id || chatId]);
        if (state.currentMsg["contact"] === "" && state.currentMsg["group" === ""]) {
          state.currentMsg = `${chatType}_${chatId}`;
        }
        else {
          state.currentMsgs = Object.assign({}, state.msgList[chatType][params.id || chatId]);
          if (state.currentMsg === `${chatType}_${chatId}`) {
             state.currentMsgs = Object.assign({}, state.msgList[chatType][chatId]); // 这里params.id在路由跳转的时候会undefind，取chatId兼容
          }
        }
      }
  ...
  actions: {
    onChangeCurrentMsg: (context, payload) => {
      context.commit("updateCurrentMsg", {
        chatType: payload.chatType,
        chatId: payload.chatId
      });
    },
    // 获取好友列表
    onGetContactUserList: async function (context, payload) {
      let token = JSON.parse(localStorage.getItem("userInfo")).token;
      let {
        data
      } = await Friends.friendsList({
        token
      });
      let userList = [];
      let user = data.data;
      user.forEach(function (item, index) {
        let data = {};
        data.jid = `1132191221113786#kanliao_${item.nickname}@easemob.com`
        // data.jid = {
        //   appKey:"1132191221113786#kanliao",
        //   domian:"easemob.com",
        //   clientResource:`${item.user_emchat_name}`,
        //   name:`${item.nickname}`
        // };
        data.uid = item.user_id;
        data.name = item.nickname;
        data.subscription = 'both';
        data.userId = item.user_emchat_name;
        userList.push(data);
      })
    ...
    // 获取群组列表
    onGetGroupUserList: async function (context, payload) {
      let token = JSON.parse(localStorage.getItem("userInfo")).token;
      let {
        data
      } = await Groups.groupList({
        token
      });
      if (data.status == 1) {
        let userList = data.data;
        userList.forEach((user, index) => {
          userList[index].name = user.group_name;
          userList[index].groupid = user.group_emchat_id;
        });
        context.commit("updateUserList", {
          userList,
          type: "groupUserList"
    ...
    onSendText: function (context, payload) {
      ...
      msgObj.set({
        msg: message,
        to: jid[chatType] === "userId" ? chatId["userId"] : chatId[jid[chatType]],
        // to: chatId[jid[chatType]],
        chatType: type,
        roomType: chatroom,
        ext: {
          "avatar": JSON.parse(localStorage.getItem("userData")).user_logo,
          "nickname": JSON.parse(localStorage.getItem("userData")).user_name
        },
        success: function () {
          context.commit("updateMsgList", {
            chatType,
            // chatId: chatId[jid[chatType]],
            chatId: jid[chatType] === "userId" ? chatId["userId"] : chatId[jid[chatType]],
            msg: message,
            bySelf: true,
            time: time,
            mid: id,
          ...
          try {
            ...
            if (msgs.length) {
              ...
              msgs.forEach((item) => {
                ...
                } else if (!item.ext.file_length && item.filename !== "audio" && item.filename.substring(
                  item.filename.length - 3) !== "mp4") { // 为图片的情况
                  msg = {
                    msg: item.url,
                    chatType: payload.isGroup ? "group" : "contact",
                    chatId: bySelf ? item.to : item.from,
