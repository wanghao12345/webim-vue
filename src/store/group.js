...
const Group = {
  ...
  actions: {
    ...
    //获取群组详情
    onGetGroupinfo: async function(context, payload) {
     ...
     let {data} = await Groups.groupInfo(params);
     if(data.status == 1){
        let  goupsInfo = data.data;
        let name = goupsInfo.group_name;
        let admin = goupsInfo.user_emchat_name;
        let desc = goupsInfo.group_about;
        let membersonly = false;
        let members = goupsInfo.head_sculpture;
    ...
    onRemoveGroupBlack: function(context, payload) {
      ...
      var options = {
        ...
        success: function(resp) {
          ...
          Vue.$store.dispatch("onGetGroupBlack", {
            select_id
          })
          this.$forceUpdate();
        },
