  ...
  <div class="messagebox" v-show="activedKey[type]!= ''">
    <div class="messagebox-header">
      ...
      <div>
        ...
        <span style="font-size: 24px;font-weight: 800;">{{`${activedKey[type].name } &nbsp;&nbsp;`}}</span>
        <span style="visibility: hidden;">{{`${activedKey[type].name } &nbsp;&nbsp; ${activedKey[type].groupid || ''}`}}</span>
    ...
    <div class="messagebox-content" ref="msgContent">
      ...
      >
        <h4 style="text-align: left;margin:0" >{{item.nickname}}</h4>
        <!-- 撤回消息 -->
        ...
        >
          <span style="user-select: none">
            ...
            <p
              ...
              v-else
              v-html="renderTxt(item.msg).replace(/\r?\n/g, '<br />')"
    ...
    <div class="messagebox-footer">
      ...
      <div class="fotter-send">
       ...
       :autosize="{ minRows: 2, maxRows: 2 }"
		    ref="txtDom"
		    />
         <a-button  style="position: absolute;bottom: 3px;right: 19px;" @click="onSendTextMsg">发送</a-button>
      </div>
    </div>
  ...
  methods: {
    ...
    select(key) {
      ...
      if (this.type === "group") {
        ...
        this.onGetCurrentChatObjMsg({ type: this.type, id: key.userId });
        setTimeout(() => {
